...
// Complete mapping from 3-letter to 2-letter ISO country codes
const iso3to2 = {
...
// All 195 countries data from the uploaded CSV
...
async function uploadAllCountries() {
  try {
    console.log('🌍 Starting complete upload of all 195 countries...')
    
    // Get current countries to avoid duplicates
    ...
    const newCountries = allCountriesData.filter(country => {
      const code2 = iso3to2[country.country_code] || country.country_code
      return !existingNames.has(country.country_name) && !existingCodes.has(code2) && !existingCodes.has(country.country_code)
    })
    ...
    // Convert to database format with 2-letter codes
    const countriesToInsert = newCountries.map(country => ({
      id: uuidv4(),
      name: country.country_name,
      country_code: iso3to2[country.country_code] || country.country_code.substring(0, 2), // Convert to 2-letter
    ...
    for (let i = 0; i < countriesToInsert.length; i += batchSize) {
      ...
      console.log(`✅ Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(countriesToInsert.length / batchSize)} (${batch.length} countries)`)
    }
    
    console.log(`🎉 Successfully added ${totalInserted} new countries!`)
    ...
    if (count < 195) {
      console.log(`⚠️  Still missing ${195 - count} countries. Let me check what's missing...`)
      ...
      const inDBNames = new Set(allInDB?.map(c => c.name) || [])
      const missing = allCountriesData.filter(c => !inDBNames.has(c.country_name))
      
      console.log('Missing countries:', missing.slice(0, 10).map(c => c.country_name))
    }
