...
// Lightweight index of all restaurants with their country, for search
export async function getRestaurantsIndex() {
  const { data, error } = await supabase
    .from('restaurants')
    .select('id, restaurant_name, location, country_id, countries!restaurants_country_id_fkey(name, country_code)')
    .order('restaurant_name')

  if (error) {
    console.error('Error in getRestaurantsIndex:', error)
    throw error
  }
  return (data || []).map(r => ({
    id: r.id,
    ...
    country_code: r.countries?.country_code || null
  }))
}
